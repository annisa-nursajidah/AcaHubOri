...
@section('content')
@php $title = 'Edit Kelas'; @endphp

<div class="max-w-2xl mx-auto">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Kelas — {{ $classroom->nama }}</h1>

    <form method="POST" action="{{ route('classrooms.update', $classroom) }}" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-6">
        @csrf @method('PUT')
        ...
        <div>
            ...
            <select name="wali_kelas_id" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition">
                ...
                @foreach($teachers as $teacher)
                    <option value="{{ $teacher->id }}" {{ old('wali_kelas_id', $classroom->wali_kelas_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->user->name }}</option>
                @endforeach
            </select>
        </div>
