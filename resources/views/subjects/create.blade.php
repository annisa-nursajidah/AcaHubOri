...
@section('content')
@php $title = 'Tambah Mata Pelajaran'; @endphp
...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-extrabold text-gray-900">Tambah Mata Pelajaran</h1>
    </div>
        ...
        <form method="POST" action="{{ route('subjects.store') }}" class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="nama" name="nama" type="text" value="{{ old('nama') }}" required class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500 transition" placeholder="Matematika">
                    @error('nama')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="kode" class="block text-sm font-medium text-gray-700 mb-1">Kode <span class="text-red-400">*</span></label>
                    <input id="kode" name="kode" type="text" value="{{ old('kode') }}" required class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500 transition" placeholder="MTK">
            ...
            <div>
                ...
                <textarea id="deskripsi" name="deskripsi" rows="3" class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500 transition resize-none" placeholder="Deskripsi singkat...">{{ old('deskripsi') }}</textarea>
                ...
                <div class="grid grid-cols-2 gap-2">
                    @foreach($teachers as $t)
                        <label class="flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-gray-200 cursor-pointer has-[:checked]:border-brand-500 has-[:checked]:bg-brand-50 transition">
                            <input type="checkbox" name="teachers[]" value="{{ $t->id }}" {{ in_array($t->id, old('teachers', [])) ? 'checked' : '' }}
                                class="rounded border-gray-300 text-brand-600 focus:ring-brand-500">
                            <span class="text-sm text-gray-700">{{ $t->user->name }}</span>
                        </label>
                    @endforeach
                </div>
                @error('teachers')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
