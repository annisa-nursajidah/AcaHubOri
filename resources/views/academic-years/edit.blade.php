...
@section('content')
@php $title = 'Edit Tahun Ajaran'; @endphp

<div class="max-w-2xl mx-auto">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Tahun Ajaran</h1>

    <form method="POST" action="{{ route('academic-years.update', $year) }}" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-6">
        @csrf @method('PUT')

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Tahun Ajaran</label>
            <input type="text" name="tahun" value="{{ old('tahun', $year->tahun) }}" placeholder="Contoh: 2025/2026"
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', $year->tanggal_mulai->format('Y-m-d')) }}"
            ...
            <div>
                ...
                <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai', $year->tanggal_selesai->format('Y-m-d')) }}"
