...
<div class="mb-6 flex items-center gap-3">
    ...
    <div>
        <h1 class="text-2xl font-black text-gray-900 tracking-tight">Buat Ujian Baru</h1>
        <p class="text-sm text-gray-500 mt-1">Konfigurasi mata pelajaran, kelas, dan durasi ujian CBT.</p>
    </div>
</div>
    ...
    <form action="{{ route('exams.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title" id="title" value="{{ old('title') }}" required
                   class="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50/50 text-sm focus:bg-white focus:outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all placeholder:text-gray-400" 
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                      class="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50/50 text-sm focus:bg-white focus:outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all placeholder:text-gray-400" 
                      placeholder="Petunjuk pengerjaan bagi siswa... (opsional)">{{ old('description') }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div>
                ...
                <div class="relative">
                    <input type="number" name="duration_minutes" id="duration_minutes" value="{{ old('duration_minutes', 60) }}" min="5" max="300" required
        ...
        <div class="bg-blue-50/50 border border-blue-100 rounded-xl p-4 text-sm text-blue-800 flex items-start gap-3">
            ...
            <p>Ujian yang baru dibuat akan berstatus <strong>Draft</strong> sehingga siswa belum bisa mengakses/melihatnya. Anda bebas memasukkan soal terlebih dahulu, lalu mengganti statusnya menjadi Aktif (Published) kelak.</p>
        </div>

        <div class="pt-4 border-t border-gray-100 flex justify-end gap-3">
            <a href="{{ route('exams.index') }}" class="px-5 py-2.5 text-sm font-semibold text-gray-600 hover:text-gray-900 bg-white border border-gray-200 hover:bg-gray-50 rounded-xl transition-colors">Batal</a>
            <button type="submit" class="px-5 py-2.5 text-sm font-semibold text-white bg-brand-600 hover:bg-brand-700 rounded-xl transition-colors shadow-sm">Simpan Ujian</button>
        </div>
    </form>
</div>
@endsection
