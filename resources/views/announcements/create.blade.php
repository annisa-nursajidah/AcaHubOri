...
@section('content')
@php $title = 'Buat Pengumuman'; @endphp
...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-extrabold text-gray-900">Buat Pengumuman</h1>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <form method="POST" action="{{ route('announcements.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <input id="judul" name="judul" type="text" value="{{ old('judul') }}" required class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500 transition" placeholder="Judul pengumuman...">
            ...
            <div>
                ...
                <textarea id="konten" name="konten" rows="6" required class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500 transition resize-none" placeholder="Tulis pengumuman...">{{ old('konten') }}</textarea>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="target" name="target" class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500 transition">
                        <option value="all" {{ old('target') === 'all' ? 'selected' : '' }}>Semua</option>
                        <option value="teacher" {{ old('target') === 'teacher' ? 'selected' : '' }}>Guru saja</option>
                        <option value="student" {{ old('target') === 'student' ? 'selected' : '' }}>Siswa saja</option>
                    </select>
                </div>
                <div class="flex items-end">
                    <label class="flex items-center gap-2.5 px-4 py-3 rounded-xl border border-gray-200 cursor-pointer has-[:checked]:border-accent-500 has-[:checked]:bg-accent-50 transition w-full">
                        <input type="checkbox" name="is_pinned" value="1" {{ old('is_pinned') ? 'checked' : '' }} class="rounded border-gray-300 text-accent-600 focus:ring-accent-500">
            ...
            <div class="flex items-center gap-3 pt-2">
                <button type="submit" class="px-6 py-3 rounded-xl bg-accent-500 text-white font-semibold text-sm hover:bg-accent-600 shadow-lg shadow-accent-500/25 transition-all">Posting</button>
